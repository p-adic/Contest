...
#define DF_OF_UNION_FOR_SET(SET)TE <TY T> IN SET& OP|=(SET& a0,CO SET& a1){for(auto& t:a1){a0 += t;}RE a0;}TE <TY T> IN SET OP|(SET a0,CO SET& a1){RE MO(a0 |= a1);}TE <TY T> IN int len(CO SET& S){RE S.SZ();}
...
DF_OF_COUT_FOR_VE(VE);DF_OF_COUT_FOR_VE(LI);DF_OF_COUT_FOR_VE(set);DF_OF_COUT_FOR_VE(unordered_set);
...
#define DF_OF_ARS_FOR_VE(V)TE <TY T> IN V<T>& OP+=(V<T>& a,CO T& t){a.push_back(t);RE a;}DF_OF_SCALAR_ACTION_FOR_VE(V,*);DF_OF_SCALAR_ACTION_FOR_VE(V,/);DF_OF_SCALAR_ACTION_FOR_VE(V,%);DF_OF_AR_FOR_VE(V,+);DF_OF_AR_FOR_VE(V,-);DF_OF_AR_FOR_VE(V,*);DF_OF_AR_FOR_VE(V,/);DF_OF_AR_FOR_VE(V,%);DF_OF_INCREMENT_FOR_VE(V,++);DF_OF_INCREMENT_FOR_VE(V,--);TE <TY T> IN V<T> OP*(CO T& scalar,V<T> v){for(auto& t:v){t *= scalar;}RE MO(v);}
DF_OF_ARS_FOR_VE(VE);DF_OF_ARS_FOR_VE(LI);TE <TY V> IN auto Get(V& a){RE[&](CRI i = 0)-> CO decldecay_t(a[0])&{RE a[i];};}TE <TY T> IN VE<T> id(CRI SZ){VE<T> AN(SZ);for(int i = 0;i < SZ;i++){AN[i]= i;}RE AN;}TE <TY T> IN VO Sort(VE<T>& a,CO bool& reversed){if(reversed){ST auto comp =[](CO T& t0,CO T& t1){RE t1 < t0;};sort(a.BE(),a.EN(),comp);}else{sort(a.BE(),a.EN());}}TE <TY T0,TY T1> IN VO Sort(VE<T0>& a,VE<T1>& b,CO bool& reversed){CO int SZ = a.SZ();AS(SZ == int(b.SZ()));VE<pair<T0,T1>> v(SZ);for(int i = 0;i < SZ;i++){v[i]={MO(a[i]),MO(b[i])};}Sort(v,reversed);for(int i = 0;i < SZ;i++){a[i]= MO(v[i].first);b[i]= MO(v[i].second);}}TE <TY T> IN VE<int> IndexSort(CO VE<T>& a,CO bool& reversed){auto index = id<int>(a.SZ());if(reversed){sort(index.BE(),index.EN(),[&](CRI i,CRI j){RE a[j]< a[i];});}else{sort(index.BE(),index.EN(),[&](CRI i,CRI j){RE a[i]< a[j];});}RE index;}
